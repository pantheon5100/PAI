...
@torch.no_grad()
def _iter_embedding_batches(data_source, batch_size=None):
    if torch.is_tensor(data_source):
        ...
        for start in range(0, total, int(batch_size)):
            ...
            batch_indices = torch.arange(start, end, dtype=torch.long).view(-1, 1)
            yield data_source[start:end], batch_indices
        return
    yield from data_source


def _reduce_memory_bank(embeddings_tensor, indices_tensor, num_cores):
    ...
    return embeddings_tensor[core_indices], indices_tensor[core_indices]


@torch.no_grad()
def create_memory_bank(model, data_source, device, num_cores=None, batch_size=None):
    model.eval()
    embeddings = []
    indices = []

    inferred_batch_size = batch_size or getattr(data_source, "batch_size", None)
    for data, batch_indices in _iter_embedding_batches(data_source, batch_size=inferred_batch_size):
    ...
    return _reduce_memory_bank(embeddings_tensor, indices_tensor, num_cores)
